      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        {viewMode === ViewMode.DASHBOARD && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              <div className="lg:col-span-2 bg-white p-6 rounded-3xl border min-h-[300px]"><h3 className="text-[10px] font-black uppercase mb-4 flex items-center gap-2"><PieChartIcon size={14} /> Komposisi SPT</h3><ResponsiveContainer width="100%" height="80%"><RePieChart><Pie data={chartData.pieData} dataKey="value" cx="50%" cy="50%" innerRadius={50} outerRadius={70}>{chartData.pieData.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip /></RePieChart></ResponsiveContainer></div>
              <div className="lg:col-span-3 bg-white p-6 rounded-3xl border min-h-[300px]"><h3 className="text-[10px] font-black uppercase mb-4 flex items-center gap-2"><Map size={14} /> Statistik Tujuan NTB</h3><ResponsiveContainer width="100%" height="80%"><ReBarChart layout="vertical" data={chartData.ntbDestStats}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 9 }} /><Tooltip /><Bar dataKey="count" fill="#4f46e5" radius={[0, 4, 4, 0]} /></ReBarChart></ResponsiveContainer></div>
            </div>
          </div>
        )}
        ...
        {viewMode === ViewMode.PRINT_MENU && (
           <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
             ...
             <div className="overflow-x-auto">
               <table className="w-full text-left">
                 <thead className="bg-slate-50 text-slate-400 text-[9px] uppercase font-black border-b border-slate-100"><tr><th className="px-6 py-4">Nomor & Tujuan</th><th className="px-6 py-4">Pejabat Pengesah (Blok II, III, IV)</th><th className="px-6 py-4 text-right">Opsi Cetak</th></tr></thead>
                     ...
                     <tr key={item.id} className="hover:bg-slate-50 transition">
                       ...
                       <td className="px-6 py-5 space-y-2">
                         {['II', 'III', 'IV'].map((blok, idx) => (
                           <div key={blok} className="flex items-center gap-2">
                             ...
                             <select className="w-full max-w-[200px] p-1.5 border border-slate-200 rounded-lg text-[9px] font-bold bg-white text-slate-700 shadow-sm" value={(item.destinationOfficialIds || [])[idx] || ''} onChange={(e) => {
                       ...
                       <td className="px-6 py-5 text-right">
                         <div className="flex gap-2 flex-wrap justify-end">
                           {[{ label: 'SPT', type: PrintType.SPT, color: 'blue' }, { label: 'SPD Dpn', type: PrintType.SPPD_FRONT, color: 'emerald' }, { label: 'SPD Blk', type: PrintType.SPPD_BACK, color: 'emerald' }, { label: 'Kuitansi', type: PrintType.KUITANSI, color: 'amber' }, { label: 'Rincian', type: PrintType.LAMPIRAN_III, color: 'purple' }, { label: 'Terima', type: PrintType.DAFTAR_PENERIMAAN, color: 'rose' }].map(btn => (<button key={btn.type} onClick={() => { setActiveAssignment(item); setPrintType(btn.type as PrintType); setViewMode(ViewMode.PRINT_PREVIEW); }} className={`px-2 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border ${btn.color === 'blue' ? 'text-blue-600 border-blue-100 bg-blue-50 hover:bg-blue-600 hover:text-white' : btn.color === 'emerald' ? 'text-emerald-600 border-emerald-100 bg-emerald-50 hover:bg-emerald-600 hover:text-white' : btn.color === 'amber' ? 'text-amber-600 border-amber-100 bg-amber-50 hover:bg-amber-600 hover:text-white' : btn.color === 'purple' ? 'text-purple-600 border-purple-100 bg-purple-50 hover:bg-purple-600 hover:text-white' : 'text-rose-600 border-rose-100 bg-rose-50 hover:bg-rose-600 hover:text-white'}`}>{btn.label}</button>))}
                         </div>
                       </td>
                     </tr>
                   ))}
