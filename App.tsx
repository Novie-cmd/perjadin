...
const App: React.FC = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center flex-col">
      ...
      <h2 className="font-black text-xl tracking-widest italic uppercase">Menghubungkan Database...</h2>
    </div>
  );
      ...
      <div className="bg-gray-100 min-h-screen">
        <div className="no-print bg-white border-b p-4 sticky top-0 flex items-center justify-between z-50 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => window.print()} className="bg-blue-600 text-white px-8 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg hover:bg-blue-700 transition">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          <div>
            ...
            <p className="text-slate-500 text-[10px] font-bold uppercase mt-1 flex items-center gap-1">
          ...
          <div className="flex items-center gap-3">
            ...
            {viewMode === ViewMode.TRAVEL_LIST && (
              <button 
                ...
                className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-black text-xs uppercase shadow-xl transition hover:bg-blue-700 flex items-center gap-2"
              >
                 ...
                 <table className="w-full text-left">
                   ...
                   <tbody className="divide-y">
                     {financialStats.subSummary.map(item => (
                       <tr key={item.code} className="hover:bg-slate-50 text-[11px] transition">
                         ...
                         <td className={`px-6 py-4 text-right font-black ${item.sisaSpd < 0 ? 'text-rose-600' : 'text-amber-600'}`}>
        ...
        {viewMode === ViewMode.OFFICIAL_LIST && (
          <OfficialForm 
            officials={officials} 
            onSave={async (o) => {
              if (supabase) {
            ...
            onDelete={async (id) => {
              if (supabase && confirm('Hapus pejabat ini?')) {
        ...
        {viewMode === ViewMode.EMPLOYEE_LIST && (
          <EmployeeForm 
            ...
            onDelete={async (id) => {
              if (supabase && confirm('Hapus data pegawai ini?')) {
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y">
                {assignments.map(a => (
                  <tr key={a.id} className="hover:bg-slate-50 transition">
                    <td className="px-6 py-5">
                      ...
                      <div className="text-[10px] text-slate-400 font-medium">{a.startDate}</div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button 
                          onClick={async () => {
                            if(supabase && confirm('Hapus data SPT ini?')) {
                ...
                {assignments.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center text-slate-400 italic">Belum ada riwayat perjalanan dinas.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {viewMode === ViewMode.MASTER_DATA && (
          <MasterDataForm 
            ...
            onDeleteCost={async (d) => {
              if(supabase && confirm('Hapus data biaya ini?')) {
                await supabase.from('master_costs').delete().eq('destination', d);
                await refreshData();
              }
            }} 
            onClearCosts={async () => {
              if(supabase && confirm('Hapus semua master biaya?')) {
            ...
            onClearSubs={async () => {
              if(supabase && confirm('Hapus semua sub kegiatan?')) {
                       ...
                       <td className="px-6 py-5 text-right">
                         <div className="flex gap-2 flex-wrap justify-end">
                           {[
                             ...
                             <button 
                               key={btn.type} 
                               onClick={() => { setActiveAssignment(item); setPrintType(btn.type as PrintType); setViewMode(ViewMode.PRINT_PREVIEW); }} 
