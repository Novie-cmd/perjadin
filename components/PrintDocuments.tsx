...
interface Props {
  ...
  isTextOnly?: boolean; // New prop for overprinting
}
...
export const SPPDBackTemplate: React.FC<Props> = ({ assignment, skpd, officials, destinationOfficials, isTextOnly }) => {
  ...
  // Helper to hide elements in text-only mode
  const showFull = (className: string) => `${className} ${isTextOnly ? 'invisible' : ''}`;

  return (
    <div className={`print-page bg-white font-['Tahoma'] text-[11pt] border border-black p-[15mm] relative leading-tight ${isTextOnly ? 'border-transparent' : ''}`}>
      <div className={showFull("flex justify-end")}>
      ...
      <div className={`space-y-0 border-t border-black ${isTextOnly ? 'border-transparent' : ''}`}>
        {['II', 'III', 'IV'].map((id) => (
          <div key={id} className={`grid grid-cols-2 border-b border-black ${isTextOnly ? 'border-transparent' : ''}`}>
            {/* Sisi Kiri: Tiba di */}
            <div className={`border-r border-black p-2 min-h-[200px] flex flex-col ${isTextOnly ? 'border-transparent' : ''}`}>
              <div className="grid grid-cols-[20px_90px_10px_1fr] gap-x-0.5">
                <span className={showFull("font-bold")}>{id}.</span>
                <span className={showFull("")}>Tiba di</span>
                <span className={showFull("")}>:</span>
                <span className={showFull("")}>{id === 'II' ? assignment.destination : ''}</span>
                
                <span className={showFull("")}></span>
                <span className={showFull("")}>Pada tanggal</span>
                <span className={showFull("")}>:</span>
                <span className={showFull("")}>{id === 'II' ? formatDateID(assignment.startDate) : ''}</span>
                
                <span className={showFull("")}></span>
                <span className={showFull("align-top")}>Kepala</span>
                <span className={showFull("align-top")}>:</span>
            ...
            <div className="p-2 min-h-[200px] flex flex-col">
              <div className="grid grid-cols-[90px_10px_1fr] gap-x-0.5">
                <span className={showFull("")}>Berangkat dari</span>
                <span className={showFull("")}>:</span>
                <span className={showFull("")}>{id === 'II' ? assignment.destination : ''}</span>
                
                <span className={showFull("")}>Ke</span>
                <span className={showFull("")}>:</span>
                <span className={showFull("")}>{id === 'II' ? (skpd.lokasi || 'Mataram') : ''}</span>
                
                <span className={showFull("")}>Pada tanggal</span>
                <span className={showFull("")}>:</span>
                <span className={showFull("")}>{id === 'II' ? formatDateID(assignment.endDate) : ''}</span>
      ...
      <div className={showFull("mt-4 pt-2 border-t border-black text-[10pt]")}>
        ...
        return (
          <div key={cost.employeeId} className="print-page bg-white text-[11pt] p-[15mm] leading-tight">
             ...
             <div className="grid grid-cols-2 gap-4 text-center mb-8">
               <div><p>Telah dibayar sejumlah :</p><p className="mb-1">Rp. {formatNumber(grandTotal)}</p><div className="min-h-[40px]"><p className="font-bold">{bendahara.jabatan},</p></div><div className="h-16"></div><p className="font-bold underline uppercase">{bendahara.name}</p><p>NIP. {bendahara.nip}</p></div>
               <div><p>{skpd.lokasi || 'Mataram'}, {formatDateID(assignment.signDate).split(' ').slice(1).join(' ')}</p><p>Telah menerima jumlah uang sebesar :</p><p className="mb-1">Rp. {formatNumber(grandTotal)}</p><div className="min-h-[40px]"><p className="font-bold">Yang menerima,</p></div><div className="h-16"></div><p className="font-bold underline uppercase">{emp.name}</p><p>NIP. {emp.nip}</p></div>
             </div>
